...
/*************************************************************************************************************/
$(document).ready(function () {
    $('.cart-product').on('mouseenter', '.cart-box', function () {
        $('#flyout-cart').addClass('active');
    });
    $('.cart-product').on('mouseleave', '.cart-box', function () {
        $('#flyout-cart').removeClass('active');
    });
    $('.cart-product').on('mouseenter', '#flyout-cart', function () {
        $('#flyout-cart').addClass('active');
    });
    $('.cart-product').on('mouseleave', '#flyout-cart', function () {
        $('#flyout-cart').removeClass('active');
    });

    if ($('.completed-page .order-code').length) {
        var data = { 'method': 'GetOrderCode' };
        $.ajax({
            cache: false,
            ...
            success: function (result) {
                if (result.success)
                    $('.completed-page .order-code').text(result.ordercode);
                else
                    setLocation(result.homepageurl);
            }
        });
    }
});
var AjaxCart = {
    ...
    effecttocart: false,
    topcartselector: '.cart-number-pd',
    topwishlistselector: '',
...
!function (a) { "function" == typeof define && define.amd ? define(["jquery"], a) : a(jQuery) }(function (a) {
    a.extend(a.fn, {
        ...
        a.extend(a.expr[":"], {
        ...
        }), a.validator = function (b, c) { this.settings = a.extend(!0, {}, a.validator.defaults, b), this.currentForm = c, this.init() }, a.validator.format = function (b, c) {
            ...
            prototype: {
                ...
                focusInvalid: function () {
                    if (this.settings.focusInvalid) try { a(this.findLastActive() || this.errorList.length && this.errorList[0].element || []).filter(":visible").focus().trigger("focusin") } catch (b) { }
                },
        ...
        }); var b, c = {}; a.ajaxPrefilter ? a.ajaxPrefilter(function (a, b, d) { var e = a.port; "abort" === a.mode && (c[e] && c[e].abort(), c[e] = d) }) : (b = a.ajax, a.ajax = function (d) { var e = ("mode" in d ? d : a.ajaxSettings).mode, f = ("port" in d ? d : a.ajaxSettings).port; return "abort" === e ? (c[f] && c[f].abort(), c[f] = b.apply(this, arguments), c[f]) : b.apply(this, arguments) })
});
...
var AjaxCheckout = {
    ...
    init: function (usepopupnotifications, shippingtotalselector, totalselector) {
        this.loadWaiting = false;
        this.usepopupnotifications = usepopupnotifications;
        this.shippingtotalselector = shippingtotalselector;
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.success == true) {
                    $(AjaxCheckout.shippingtotalselector).html(result.shippingtotalsectionhtml);
                    $(AjaxCheckout.totalselector).html(result.totalsectionhtml);
                }
            },
            complete: this.resetLoadWaiting,
            error: this.ajaxFailure
    ...
    saveorder: function (savetodb, redirect) {
        ...
        $("#aspnetForm").validate({
            ...
            messages: {
                ...
                //ShippingMethod: "Vui lòng chọn Phương thức vận chuyển",
